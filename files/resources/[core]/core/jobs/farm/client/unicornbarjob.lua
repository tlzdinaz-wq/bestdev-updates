...
-- Props statiques à spawn (visibles par tous les joueurs)
local PropsToSpawn = {
    { model = "prop_ind_mech_01c", coords = vector4(942.19, -2160.50, 30.19, 180.0) },
}
...
-- Spawn les props au démarrage
Citizen.CreateThread(function()
    Wait(1000)
    SpawnStaticProps()
end)

-- Cleanup quand la resource s'arrête
AddEventHandler("onResourceStop", function(resourceName)
    if GetCurrentResourceName() ~= resourceName then return end
    CleanupStaticProps()
end)

RegisterNetEvent("farm:unicorn:sellingPed", function(pos)
    if sellingPed then
...
RegisterNetEvent("farm:unicorn:syncZones", function(zones)
    if zones then
...
Citizen.CreateThread(function()
    while true do
        ...
        local hasGoodJob = VFW.PlayerData and VFW.PlayerData.job and VFW.PlayerData.job.name == "unicorn"
        local isInVehicle = IsPedInAnyVehicle(ped, false)

        -- 1. LOGIQUE DE RÉCOLTE (Points)
        local harvestPoints = dynamicHarvestPoints or UnicornBarConfig.harvest
        ...
        if nearestDist <= 2.0 and hasGoodJob and not isInVehicle then
            wait = 0
            VFW.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour récolter de l'orge")
            ...
            if VFW.Interact.JustReleased(0, 38) and not inHarvest then
                ...
                local success = VFW.Nui.ProgressBar("Récolte de l'orge...", 3000)

                ClearPedTasksImmediately(ped)
                FreezeEntityPosition(ped, false)

                if success then
                    TriggerServerEvent("farm:unicorn:give", "harvest")
                end
        ...
        if not processingPoints or #processingPoints == 0 then
            processingPoints = {}
            for _, coord in pairs(UnicornBarConfig.processing) do
            ...
            if dist < 2.0 and hasGoodJob and not isInVehicle then
                ...
                if dist < 1.5 then
                    ...
                    if VFW.Interact.JustReleased(0, 38) then
                        for barley_type, _ in pairs(UnicornBarConfig.items.process) do
                            local canProcess = TriggerServerCallback("farm:unicorn:canProcess", "process", barley_type)
                            ...
                            if canProcess then
                                ...
                                if success then
                                    TriggerServerEvent("farm:unicorn:give", "process", barley_type)
                                end
                                break
                            else
        ...
        if sellingPed and #(GetEntityCoords(sellingPed) - pCoords) < 2.5 and not isInVehicle then
            ...
            if #(GetEntityCoords(sellingPed) - pCoords) < 1.5 then
                ...
                if VFW.Interact.JustReleased(0, 38) and not inSelling then
                    local canSell = TriggerServerCallback("farm:unicorn:canProcess", "selling")
                    if canSell then
                        inSelling = true
                        ...
                        Citizen.CreateThread(function()
                            ...
                            if success then
                                TriggerServerEvent("farm:unicorn:selling")
                            else
