...
RegisterNetEvent("farm:asgard:sellingPed", function(pos)
    if sellingPed then
...
RegisterNetEvent("farm:asgard:syncZones", function(zones)
    if zones then
...
Citizen.CreateThread(function()
    while true do
        ...
        local hasGoodJob = VFW.PlayerData and VFW.PlayerData.job and VFW.PlayerData.job.name == "asgard"
        local isInVehicle = IsPedInAnyVehicle(ped, false)

        -- 1. LOGIQUE DE RECOLTE (Points)
        local harvestPoints = dynamicHarvestPoints or AsgardBarConfig.harvest
        ...
        if nearestDist <= 2.0 and hasGoodJob and not isInVehicle then
            ...
            if VFW.Interact.JustReleased(0, 38) and not inHarvest then
                ...
                if success then
                    TriggerServerEvent("farm:asgard:give", "harvest")
                end
        ...
        if not processingPoints or #processingPoints == 0 then
            processingPoints = {}
            for _, coord in pairs(AsgardBarConfig.processing) do
            ...
            if dist < 2.0 and hasGoodJob and not isInVehicle then
                ...
                if dist < 1.5 then
                    ...
                    if VFW.Interact.JustReleased(0, 38) then
                        for barley_type, _ in pairs(AsgardBarConfig.items.process) do
                            local canProcess = TriggerServerCallback("farm:asgard:canProcess", "process", barley_type)
                            ...
                            if canProcess then
                                ...
                                if success then
                                    TriggerServerEvent("farm:asgard:give", "process", barley_type)
                                end
                                break
                            else
        ...
        if sellingPed and #(GetEntityCoords(sellingPed) - pCoords) < 2.5 and not isInVehicle then
            ...
            if #(GetEntityCoords(sellingPed) - pCoords) < 1.5 then
                ...
                if VFW.Interact.JustReleased(0, 38) and not inSelling then
                    local canSell = TriggerServerCallback("farm:asgard:canProcess", "selling")
                    if canSell then
                        inSelling = true
                        ...
                        Citizen.CreateThread(function()
                            ...
                            if success then
                                TriggerServerEvent("farm:asgard:selling")
                            else
