...
RegisterNetEvent("farm:yellowjack:sellingPed", function(pos)
    if sellingPed then
...
RegisterNetEvent("farm:yellowjack:syncZones", function(zones)
    if zones then
...
Citizen.CreateThread(function()
    while true do
        ...
        local hasGoodJob = VFW.PlayerData and VFW.PlayerData.job and VFW.PlayerData.job.name == "yellowjack"
        local isInVehicle = IsPedInAnyVehicle(ped, false)

        -- 1. LOGIQUE DE RECOLTE (Points)
        local harvestPoints = dynamicHarvestPoints or YellowJackBarConfig.harvest
        ...
        if nearestDist <= 2.0 and hasGoodJob and not isInVehicle then
            ...
            if VFW.Interact.JustReleased(0, 38) and not inHarvest then
                ...
                if success then
                    TriggerServerEvent("farm:yellowjack:give", "harvest")
                end
        ...
        if not processingPoints or #processingPoints == 0 then
            processingPoints = {}
            for _, coord in pairs(YellowJackBarConfig.processing) do
            ...
            if dist < 2.0 and hasGoodJob and not isInVehicle then
                ...
                if dist < 1.5 then
                    ...
                    if VFW.Interact.JustReleased(0, 38) then
                        for barley_type, _ in pairs(YellowJackBarConfig.items.process) do
                            local canProcess = TriggerServerCallback("farm:yellowjack:canProcess", "process", barley_type)
                            ...
                            if canProcess then
                                ...
                                if success then
                                    TriggerServerEvent("farm:yellowjack:give", "process", barley_type)
                                end
                                break
                            else
        ...
        if sellingPed and #(GetEntityCoords(sellingPed) - pCoords) < 2.5 and not isInVehicle then
            ...
            if #(GetEntityCoords(sellingPed) - pCoords) < 1.5 then
                ...
                if VFW.Interact.JustReleased(0, 38) and not inSelling then
                    local canSell = TriggerServerCallback("farm:yellowjack:canProcess", "selling")
                    if canSell then
                        inSelling = true
                        ...
                        Citizen.CreateThread(function()
                            ...
                            if success then
                                TriggerServerEvent("farm:yellowjack:selling")
                            else
