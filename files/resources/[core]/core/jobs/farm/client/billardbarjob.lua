...
RegisterNetEvent("farm:billard:sellingPed", function(pos)
    if sellingPed then
...
RegisterNetEvent("farm:billard:syncZones", function(zones)
    if zones then
...
Citizen.CreateThread(function()
    while true do
        ...
        local hasGoodJob = VFW.PlayerData and VFW.PlayerData.job and VFW.PlayerData.job.name == "billard"
        local isInVehicle = IsPedInAnyVehicle(ped, false)

        -- 1. LOGIQUE DE RECOLTE (Points)
        local harvestPoints = dynamicHarvestPoints or BillardBarConfig.harvest
        ...
        if nearestDist <= 2.0 and hasGoodJob and not isInVehicle then
            ...
            if VFW.Interact.JustReleased(0, 38) and not inHarvest then
                ...
                if success then
                    TriggerServerEvent("farm:billard:give", "harvest")
                end
        ...
        if not processingPoints or #processingPoints == 0 then
            processingPoints = {}
            for _, coord in pairs(BillardBarConfig.processing) do
            ...
            if dist < 2.0 and hasGoodJob and not isInVehicle then
                ...
                if dist < 1.5 then
                    ...
                    if VFW.Interact.JustReleased(0, 38) then
                        for barley_type, _ in pairs(BillardBarConfig.items.process) do
                            local canProcess = TriggerServerCallback("farm:billard:canProcess", "process", barley_type)
                            ...
                            if canProcess then
                                ...
                                if success then
                                    TriggerServerEvent("farm:billard:give", "process", barley_type)
                                end
                                break
                            else
        ...
        if sellingPed and #(GetEntityCoords(sellingPed) - pCoords) < 2.5 and not isInVehicle then
            ...
            if #(GetEntityCoords(sellingPed) - pCoords) < 1.5 then
                ...
                if VFW.Interact.JustReleased(0, 38) and not inSelling then
                    local canSell = TriggerServerCallback("farm:billard:canProcess", "selling")
                    if canSell then
                        inSelling = true
                        ...
                        Citizen.CreateThread(function()
                            ...
                            if success then
                                TriggerServerEvent("farm:billard:selling")
                            else
