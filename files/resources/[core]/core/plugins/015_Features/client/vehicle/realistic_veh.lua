    ...
    CreateThread(function()
        while state do
            ...
            if isPedDrivingAVehicle() then
                ...
                healthEngineDeltaScaled = healthEngineDelta * RealisticVeh.damageFactorEngine * (RealisticVeh.classDamageMultiplier[vehicleClass] or 1.0)
                ...
                healthBodyDeltaScaled = healthBodyDelta * RealisticVeh.damageFactorBody * (RealisticVeh.classDamageMultiplier[vehicleClass] or 1.0)
                ...
                healthPetrolTankDeltaScaled = healthPetrolTankDelta * RealisticVeh.damageFactorPetrolTank * (RealisticVeh.classDamageMultiplier[vehicleClass] or 1.0)

                if Entity(vehicle).state.engineDestroyed then
                    SetVehicleEngineHealth(vehicle, 0.0)
                    SetVehicleEngineOn(vehicle, false, true, true)
                    SetVehicleUndriveable(vehicle, true)
                    healthEngineLast = 0.0
                    healthEngineNew = 0.0
                elseif healthEngineCurrent > RealisticVeh.engineSafeGuard+1 then
                    SetVehicleUndriveable(vehicle,false)
                elseif RealisticVeh.limpMode == false then
                    SetVehicleUndriveable(vehicle,true)
                end
                ...
                lastVehicle=vehicle
                if RealisticVeh.randomTireBurstInterval ~= 0 and GetEntitySpeed(vehicle) > 10 then
                    tireBurstLottery()
                end
            else
                if pedInSameVehicleLast == true then
