...
local function CreateDeliveryBlip(coords, npcName)
    ...
    local blipName = (npcName or "Client") .. " - Livraison Bean Coffee"
...
local function EndConversation()
    ...
    SendNUIMessage({
        action = "bean_coffee:delivery:closeConversation"
    })
    VFW.Nui.Focus(false)
end
...
local function ShowDeliveryOrder()
    ...
    SendNUIMessage({
        ...
        data = {
            jobLogo = BEAN_COFFEE_LOGO,
            jobLabel = "Bean Coffee",
            npcName = name,
...
local function SpawnDeliveryNpc(clientData)
    ...
    local defaultModel = GetHashKey(BeanCoffeeConfig.DeliveryDefaultModel)
    local modelName = clientData.npcModel or BeanCoffeeConfig.DeliveryDefaultModel
...
local function StartConversation()
    ...
    local canDeliver = TriggerServerCallback("bean_coffee:delivery:checkItems")

    SendNUIMessage({
        action = "bean_coffee:delivery:openConversation",
        data = {
            npcName = npcName,
            orderItems = deliveryOrderItems,
...
local function HandleDeliveryResult(success, reward, tip, societyGain)
    if success then
        ...
        VFW.ShowNotification({
            type = "JOB", title = "Bean Coffee", subtitle = "Livraison",
            image = BEAN_COFFEE_LOGO,
            content = msg
        })
        ...
        TriggerServerCallback("bean_coffee:delivery:end")
...
RegisterNUICallback("bean_coffee:delivery:conversationChoice", function(data, cb)
    if data.choice == "deliver" then
        local success, reward, tipOrMissing, societyGain = TriggerServerCallback("bean_coffee:delivery:deliver")
        if success then
            SendNUIMessage({
                action = "bean_coffee:delivery:deliveryResult",
...
RegisterNUICallback("bean_coffee:delivery:continueChoice", function(data, cb)
    EndConversation()

    if data.choice == "continue" then
        local success, result = TriggerServerCallback("bean_coffee:delivery:next")
        if success then
            RemoveDeliveryBlip()
            RemoveDeliveryNpc()
...
function BeanCoffee_StartDelivery(jobName)
    ...
    if loc then
        ...
        if refCoords then
            ...
            local maxDist = BeanCoffeeConfig.DeliveryStartMaxDistance or 50.0
    ...
    local requiredVehicles = BeanCoffeeConfig.DeliveryVehicleModels
    if requiredVehicles and #requiredVehicles > 0 then
        local ped = PlayerPedId()
        if not IsPedInAnyVehicle(ped, false) then
            VFW.ShowNotification({ type = "JOB", title = "Bean Coffee", subtitle = "Livraison", image = BEAN_COFFEE_LOGO,
    ...
    local success, result = TriggerServerCallback("bean_coffee:delivery:start", jobName)
    if not success then
        VFW.ShowNotification({
            type = "JOB", title = "Bean Coffee", subtitle = "Livraison",
            image = BEAN_COFFEE_LOGO,
...
function BeanCoffee_EndDelivery()
...
function BeanCoffee_GetDeliveryState()
    return {
...
CreateThread(function()
    ...
    local jobNames = {}
    for jobName, _ in pairs(BeanCoffeeConfig.Locations) do
        table.insert(jobNames, jobName)
    end

    registry.register(jobNames, function(menu)
        local state = BeanCoffee_GetDeliveryState()

        if not state.active then
            menu.Button(
                "Lancer une livraison",
                "Démarrer une mission de livraison Bean Coffee",
                nil,
                "arrow",
                false,
                function()
                    ...
                    if jobName then
                        BeanCoffee_StartDelivery(jobName)
                    end
                    menu.close()
                end
            )
...
RegisterNUICallback("bossPanel:clearBeanCoffeeLogs", function(data, cb)
    local deleted = TriggerServerCallback("bean_coffee:delivery:clearLogs")
    if not deleted then
        VFW.ShowNotification({
            type = "JOB", title = "Bean Coffee", subtitle = "Administration",
            image = BEAN_COFFEE_LOGO,
    ...
    while true do
        ...
        if deliveryActive and deliveryClientData then
            ...
            if deliveryClientData then
                ...
                local spawnDist = BeanCoffeeConfig.DeliveryNpcSpawnDistance or 50.0
