...
local function getPlayerBeanCoffeeJob()
    ...
    if BeanCoffeeConfig.Locations[jobName] then
        return jobName
    end
    return nil
end
    ...
    for jobName, loc in pairs(BeanCoffeeConfig.Locations) do
        if loc.Machine then
            for _, slotData in pairs(loc.Machine.slots) do
                table.insert(stations, { coords = slotData.pos, radius = loc.Machine.radius, label = "Appuyez sur ~INPUT_CONTEXT~ pour utiliser la ~b~Machine", jobName = jobName, action = function() BeanCoffee_OpenMachine(jobName) end })
            end
        end
        if loc.CraftingTable then
            for _, slotData in pairs(loc.CraftingTable.slots) do
                table.insert(stations, { coords = slotData.pos, radius = loc.CraftingTable.radius, label = "Appuyez sur ~INPUT_CONTEXT~ pour utiliser le ~b~Plan de travail", jobName = jobName, action = function() BeanCoffee_OpenCrafting(jobName) end })
            end
        end
        if loc.GranitaStation then
            for _, slotData in pairs(loc.GranitaStation.slots) do
                table.insert(stations, { coords = slotData.pos, radius = loc.GranitaStation.radius, label = "Appuyez sur ~INPUT_CONTEXT~ pour utiliser la ~b~Station Granita", jobName = jobName, action = function() BeanCoffee_OpenGranita(jobName) end })
            end
        end
    end
end
...
local function startLoop()
    ...
    CreateThread(function()
        ...
        while active do
            ...
            local playerJob = getPlayerBeanCoffeeJob()
...
RegisterNetEvent("vfw:setJob", function()
    Wait(500)
    if getPlayerBeanCoffeeJob() then
        startLoop()
    else
        stopLoop()
    end
