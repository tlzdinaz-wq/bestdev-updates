...
--- Open LifeInvader Panel (Régie TV)
VFW.Nui.lifeinvaderPanel = function(visible)
    if visible then
        local announcements, history = splitAnnouncements(TriggerServerCallback("lifeinvader:getAnnouncements"))
        local societyImage = TriggerServerCallback("lifeinvader:getSocietyImage") or ""
        lifeinvaderImage = societyImage

        local society = TriggerServerCallback("lifeinvader:getSocietyCustom") or {}
        local lifeinvaderPerms = society.lifeinvaderPerms or { create = 0, edit = 0, schedule = 0, delete = 0 }

        SendNUIMessage({
            action = "nui:LifeInvaderPanel:data",
            data = {
                job = VFW.PlayerData.job.name,
                announcements = announcements or {},
                history = history or {},
                lifeinvaderPerms = lifeinvaderPerms,
    ...
    SendNUIMessage({
        action = "nui:LifeInvaderPanel:visible",
        data = visible
    })
...
function OpenlifeinvaderPanel()
    VFW.Nui.lifeinvaderPanel(true)
end

RegisterNuiCallback("nui:closeLifeInvaderPanel", function()
    VFW.Nui.lifeinvaderPanel(false)
    isPanelOpen = false
    VFW.ClearPreview()
end)

-- Refresh panel data when announcements change (triggered by server)
RegisterNetEvent("lifeinvader:refreshPanel")
AddEventHandler("lifeinvader:refreshPanel", function()
    if not isPanelOpen then return end
