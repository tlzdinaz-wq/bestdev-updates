...
local function getStationConfig(stationType, jobName)
    ...
    if stationType == "broth" then
        return loc.BrothStation
    end
    return nil
end
...
local function closeCookingMenu()
    ...
    SendNUIMessage({ action = "noodle:cooking:visible", data = false })
end
...
local function hideProgress()
    SendNUIMessage({ action = "noodle:progress:hide", data = {} })
end
...
local function startCookingLoop(stationType, jobName, inputItem, outputItem, quantity)
    ...
    local animCfg = NoodleConfig.SharedAnim[stationType]
    ...
    for _, slot in ipairs(sortedSlots) do
        success, errMsg = TriggerServerCallback("noodle:cooking:claimSlot", jobName, stationType, slot.name, inputItem)
        if success then
            slotName = slot.name
            break
        end
    ...
    if not slotName then
        VFW.ShowNotification({ type = "JOB", title = "Noodle", subtitle = "Cuisine", image = NOODLE_LOGO, content = errMsg or "Tous les postes sont occupés." })
        return
    end
    ...
    for i = 1, quantity do
        local claimOk = TriggerServerCallback("noodle:cooking:cookOne", jobName, stationType, slotName, inputItem)
        if not claimOk then
            if cooked == 0 then
                VFW.ShowNotification({ type = "JOB", title = "Noodle", subtitle = "Cuisine", image = NOODLE_LOGO, content = "Il vous manque l'ingrédient nécessaire." })
            end
            break
        end
        ...
        if not completed then
            VFW.ShowNotification({ type = "JOB", title = "Noodle", subtitle = "Cuisine", image = NOODLE_LOGO, content = "La cuisson a été annulée." })
            break
        end

        local result = TriggerServerCallback("noodle:cooking:completeOne", jobName, stationType, slotName, inputItem, outputItem)
        if result then
            cooked = cooked + 1
        else
            VFW.ShowNotification({ type = "JOB", title = "Noodle", subtitle = "Cuisine", image = NOODLE_LOGO, content = "Une erreur est survenue lors de la cuisson." })
            break
        end
    end
    ...
    TriggerServerCallback("noodle:cooking:cancel", jobName, stationType, slotName)

    if cooked > 0 then
        VFW.ShowNotification({ type = "JOB", title = "Noodle", subtitle = "Cuisine", image = NOODLE_LOGO, content = "La cuisson est terminée ! (" .. cooked .. "/" .. quantity .. ")" })
    end

    isCooking = false
end

function Noodle_StartCooking(stationType, jobName)
    if isCooking or cookingOpen then return end

    local recipes = TriggerServerCallback("noodle:cooking:getRecipes", jobName, stationType)
    if not recipes then
    ...
    if #recipes == 1 then
        ...
        if not recipe.hasInput or recipe.playerHas < 1 then
            VFW.ShowNotification({ type = "JOB", title = "Noodle", subtitle = "Cuisine", image = NOODLE_LOGO, content = "Vous ne possédez pas l'ingrédient nécessaire." })
            return
        end
    ...
    SendNUIMessage({ action = "noodle:cooking:visible", data = true })
    SendNUIMessage({
        action = "noodle:cooking:data",
        data = {
...
RegisterNUICallback("noodle:cooking:select", function(data, cb)
...
RegisterNUICallback("noodle:cooking:close", function(_, cb)
    closeCookingMenu()
    cb({ ok = true })
end)

RegisterNetEvent("noodle:cooking:slotUpdate", function(stationType, slotName, occupied)
end)
