...
local function CreateDeliveryBlip(coords, npcName)
    ...
    local blipName = (npcName or "Client") .. " - Livraison Pizzeria"
...
local function EndConversation()
    ...
    SendNUIMessage({
        action = "pizzeria:delivery:closeConversation"
    })
    VFW.Nui.Focus(false)
end
...
local function SpawnDeliveryNpc(clientData)
    ...
    local defaultModel = GetHashKey(PizzeriaConfig.DeliveryDefaultModel)
    local modelName = clientData.npcModel or PizzeriaConfig.DeliveryDefaultModel
...
local function StartConversation()
    ...
    local canDeliver = TriggerServerCallback("pizzeria:delivery:checkItems")

    SendNUIMessage({
        action = "pizzeria:delivery:openConversation",
        data = {
            npcName = npcName,
            orderItems = deliveryOrderItems,
...
local function HandleDeliveryResult(success, reward, tip, societyGain)
    if success then
        ...
        VFW.ShowNotification({
            type = "JOB", title = "Pizzeria", subtitle = "Livraison",
            image = PIZZERIA_LOGO,
            content = msg
        })
        ...
        TriggerServerCallback("pizzeria:delivery:end")
...
RegisterNUICallback("pizzeria:delivery:conversationChoice", function(data, cb)
    if data.choice == "deliver" then
        local success, reward, tipOrMissing, societyGain = TriggerServerCallback("pizzeria:delivery:deliver")
        if success then
            SendNUIMessage({
                action = "pizzeria:delivery:deliveryResult",
...
RegisterNUICallback("pizzeria:delivery:continueChoice", function(data, cb)
    EndConversation()

    if data.choice == "continue" then
        local success, result = TriggerServerCallback("pizzeria:delivery:next")
        if success then
            RemoveDeliveryBlip()
            RemoveDeliveryNpc()
...
function Pizzeria_StartDelivery(jobName)
    ...
    if loc then
        ...
        if refCoords then
            ...
            local maxDist = PizzeriaConfig.DeliveryStartMaxDistance or 50.0
            if dist > maxDist then
                VFW.ShowNotification({
                    type = "JOB", title = "Pizzeria", subtitle = "Livraison",
                    image = PIZZERIA_LOGO,
                    content = "Vous êtes trop loin de la pizzeria pour recevoir la livraison."
                })
                return
            end
        end
    ...
    local requiredVehicles = PizzeriaConfig.DeliveryVehicleModels
    if requiredVehicles and #requiredVehicles > 0 then
        local ped = PlayerPedId()
        if not IsPedInAnyVehicle(ped, false) then
            VFW.ShowNotification({ type = "JOB", title = "Pizzeria", subtitle = "Livraison", image = PIZZERIA_LOGO,
    ...
    local success, result = TriggerServerCallback("pizzeria:delivery:start", jobName)
    if not success then
        VFW.ShowNotification({
            type = "JOB", title = "Pizzeria", subtitle = "Livraison",
            image = PIZZERIA_LOGO,
...
function Pizzeria_GetDeliveryState()
    return {
...
CreateThread(function()
    ...
    local jobNames = {}
    for jobName, _ in pairs(PizzeriaConfig.Locations) do
        table.insert(jobNames, jobName)
    end

    registry.register(jobNames, function(menu)
        local state = Pizzeria_GetDeliveryState()

        if not state.active then
            menu.Button(
                "Lancer une livraison",
                "Démarrer une mission de livraison Pizzeria",
                nil,
                "arrow",
                false,
                function()
                    ...
                    if jobName then
                        Pizzeria_StartDelivery(jobName)
                    end
                    menu.close()
                end
            )
...
RegisterNUICallback("bossPanel:clearPizzeriaLogs", function(data, cb)
    local deleted = TriggerServerCallback("pizzeria:delivery:clearLogs")
    if not deleted then
        VFW.ShowNotification({
            type = "JOB", title = "Pizzeria", subtitle = "Administration",
            image = PIZZERIA_LOGO,
    ...
    while true do
        ...
        if deliveryActive and deliveryClientData then
            ...
            if deliveryClientData then
                ...
                local spawnDist = PizzeriaConfig.DeliveryNpcSpawnDistance or 50.0
