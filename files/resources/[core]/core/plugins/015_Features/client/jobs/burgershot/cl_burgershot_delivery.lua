...
local function CreateDeliveryBlip(coords, npcName)
    ...
    local blipName = (npcName or "Client") .. " - Livraison BurgerShot"
...
local function EndConversation()
    ...
    SendNUIMessage({
        action = "burgershot:delivery:closeConversation"
    })
    VFW.Nui.Focus(false)
end
...
local function SpawnDeliveryNpc(clientData)
    ...
    local defaultModel = GetHashKey(BurgerShotConfig.DeliveryDefaultModel)
    local modelName = clientData.npcModel or BurgerShotConfig.DeliveryDefaultModel
...
local function StartConversation()
    ...
    local canDeliver = TriggerServerCallback("burgershot:delivery:checkItems")

    SendNUIMessage({
        action = "burgershot:delivery:openConversation",
        data = {
            npcName = npcName,
            orderItems = deliveryOrderItems,
...
local function HandleDeliveryResult(success, reward, tip, societyGain)
    if success then
        ...
        VFW.ShowNotification({
            type = "JOB", title = "BurgerShot", subtitle = "Livraison",
            image = BURGERSHOT_LOGO,
            content = msg
        })
        ...
        TriggerServerCallback("burgershot:delivery:end")
...
RegisterNUICallback("burgershot:delivery:conversationChoice", function(data, cb)
    if data.choice == "deliver" then
        local success, reward, tipOrMissing, societyGain = TriggerServerCallback("burgershot:delivery:deliver")
        if success then
            SendNUIMessage({
                action = "burgershot:delivery:deliveryResult",
...
RegisterNUICallback("burgershot:delivery:continueChoice", function(data, cb)
    EndConversation()

    if data.choice == "continue" then
        local success, result = TriggerServerCallback("burgershot:delivery:next")
        if success then
            RemoveDeliveryBlip()
            RemoveDeliveryNpc()
...
function BurgerShot_StartDelivery(jobName)
    ...
    if loc then
        local refCoords = loc.Machine and loc.Machine.coords
        if refCoords then
            local playerCoords = GetEntityCoords(PlayerPedId())
            local dist = #(playerCoords - refCoords)
            local maxDist = BurgerShotConfig.DeliveryStartMaxDistance or 50.0
            if dist > maxDist then
                VFW.ShowNotification({
                    type = "JOB", title = "BurgerShot", subtitle = "Livraison",
                    image = BURGERSHOT_LOGO,
                    content = "Vous êtes trop loin de l'antenne du BurgerShot pour recevoir la livraison."
                })
                return
            end
        end
    ...
    local requiredVehicles = BurgerShotConfig.DeliveryVehicleModels
    if requiredVehicles and #requiredVehicles > 0 then
        local ped = PlayerPedId()
        if not IsPedInAnyVehicle(ped, false) then
            VFW.ShowNotification({ type = "JOB", title = "BurgerShot", subtitle = "Livraison", image = BURGERSHOT_LOGO,
    ...
    local success, result = TriggerServerCallback("burgershot:delivery:start", jobName)
    if not success then
        VFW.ShowNotification({
            type = "JOB", title = "BurgerShot", subtitle = "Livraison",
            image = BURGERSHOT_LOGO,
...
function BurgerShot_EndDelivery()
    if not deliveryActive then return end
    local delivered = deliveryDelivered
    local jobName = deliveryJobName
    CleanupDelivery()
    TriggerServerCallback("burgershot:delivery:end")
    VFW.ShowNotification({
        type = "JOB", title = "BurgerShot", subtitle = "Livraison",
...
function BurgerShot_GetDeliveryState()
    return {
...
CreateThread(function()
    ...
    local jobNames = {}
    for jobName, _ in pairs(BurgerShotConfig.Locations) do
        table.insert(jobNames, jobName)
    end

    registry.register(jobNames, function(menu)
        local state = BurgerShot_GetDeliveryState()

        if not state.active then
            menu.Button(
                "Lancer une livraison",
                "Démarrer une mission de livraison BurgerShot",
                nil,
                "arrow",
                false,
                function()
                    ...
                    if jobName then
                        BurgerShot_StartDelivery(jobName)
                    end
                    menu.close()
                end
            )
...
RegisterNUICallback("bossPanel:clearBurgerShotLogs", function(data, cb)
    local deleted = TriggerServerCallback("burgershot:delivery:clearLogs")
    if not deleted then
        VFW.ShowNotification({
            type = "JOB", title = "BurgerShot", subtitle = "Administration",
            image = BURGERSHOT_LOGO,
    ...
    while true do
        ...
        if deliveryActive and deliveryClientData then
            ...
            if deliveryClientData then
                ...
                local spawnDist = BurgerShotConfig.DeliveryNpcSpawnDistance or 50.0
