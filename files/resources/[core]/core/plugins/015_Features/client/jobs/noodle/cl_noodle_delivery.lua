...
local function CreateDeliveryBlip(coords, npcName)
    ...
    local blipName = (npcName or "Client") .. " - Livraison Noodle"
...
local function EndConversation()
    ...
    SendNUIMessage({
        action = "noodle:delivery:closeConversation"
    })
    VFW.Nui.Focus(false)
end
...
local function SpawnDeliveryNpc(clientData)
    ...
    local defaultModel = GetHashKey(NoodleConfig.DeliveryDefaultModel)
    local modelName = clientData.npcModel or NoodleConfig.DeliveryDefaultModel
...
local function StartConversation()
    ...
    local canDeliver = TriggerServerCallback("noodle:delivery:checkItems")

    SendNUIMessage({
        action = "noodle:delivery:openConversation",
        data = {
            npcName = npcName,
            orderItems = deliveryOrderItems,
...
local function HandleDeliveryResult(success, reward, tip, societyGain)
    if success then
        ...
        VFW.ShowNotification({
            type = "JOB", title = "Noodle", subtitle = "Livraison",
            image = NOODLE_LOGO,
            content = msg
        })
        ...
        TriggerServerCallback("noodle:delivery:end")
...
RegisterNUICallback("noodle:delivery:conversationChoice", function(data, cb)
    if data.choice == "deliver" then
        local success, reward, tipOrMissing, societyGain = TriggerServerCallback("noodle:delivery:deliver")
        if success then
            SendNUIMessage({
                action = "noodle:delivery:deliveryResult",
...
RegisterNUICallback("noodle:delivery:continueChoice", function(data, cb)
    EndConversation()

    if data.choice == "continue" then
        local success, result = TriggerServerCallback("noodle:delivery:next")
        if success then
            RemoveDeliveryBlip()
            RemoveDeliveryNpc()
...
function Noodle_StartDelivery(jobName)
    ...
    if loc then
        ...
        if refCoords then
            ...
            local maxDist = NoodleConfig.DeliveryStartMaxDistance or 50.0
    ...
    local requiredVehicles = NoodleConfig.DeliveryVehicleModels
    if requiredVehicles and #requiredVehicles > 0 then
        local ped = PlayerPedId()
        if not IsPedInAnyVehicle(ped, false) then
            VFW.ShowNotification({ type = "JOB", title = "Noodle", subtitle = "Livraison", image = NOODLE_LOGO,
    ...
    local success, result = TriggerServerCallback("noodle:delivery:start", jobName)
    if not success then
        VFW.ShowNotification({
            type = "JOB", title = "Noodle", subtitle = "Livraison",
            image = NOODLE_LOGO,
...
function Noodle_GetDeliveryState()
    return {
...
CreateThread(function()
    ...
    local jobNames = {}
    for jobName, _ in pairs(NoodleConfig.Locations) do
        table.insert(jobNames, jobName)
    end

    registry.register(jobNames, function(menu)
        local state = Noodle_GetDeliveryState()

        if not state.active then
            menu.Button(
                "Lancer une livraison",
                "Démarrer une mission de livraison Noodle",
                nil,
                "arrow",
                false,
                function()
...
RegisterNUICallback("bossPanel:clearNoodleLogs", function(data, cb)
    local deleted = TriggerServerCallback("noodle:delivery:clearLogs")
    if not deleted then
        VFW.ShowNotification({
            type = "JOB", title = "Noodle", subtitle = "Administration",
            image = NOODLE_LOGO,
    ...
    while true do
        ...
        if deliveryActive and deliveryClientData then
            ...
            if deliveryClientData then
                ...
                local spawnDist = NoodleConfig.DeliveryNpcSpawnDistance or 50.0
