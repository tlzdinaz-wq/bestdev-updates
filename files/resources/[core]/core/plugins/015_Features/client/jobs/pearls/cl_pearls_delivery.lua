...
local function CreateDeliveryBlip(coords, npcName)
    ...
    local blipName = (npcName or "Client") .. " - Livraison Pearls"
...
local function EndConversation()
    ...
    SendNUIMessage({
        action = "pearls:delivery:closeConversation"
    })
    VFW.Nui.Focus(false)
end
...
local function SpawnDeliveryNpc(clientData)
    ...
    local defaultModel = GetHashKey(PearlsConfig.DeliveryDefaultModel)
    local modelName = clientData.npcModel or PearlsConfig.DeliveryDefaultModel
...
local function StartConversation()
    ...
    local canDeliver = TriggerServerCallback("pearls:delivery:checkItems")

    SendNUIMessage({
        action = "pearls:delivery:openConversation",
        data = {
            npcName = npcName,
            orderItems = deliveryOrderItems,
...
local function HandleDeliveryResult(success, reward, tip, societyGain)
    if success then
        ...
        VFW.ShowNotification({
            type = "JOB", title = "Pearls", subtitle = "Livraison",
            image = PEARLS_LOGO,
            content = msg
        })
        ...
        TriggerServerCallback("pearls:delivery:end")
...
RegisterNUICallback("pearls:delivery:conversationChoice", function(data, cb)
    if data.choice == "deliver" then
        local success, reward, tipOrMissing, societyGain = TriggerServerCallback("pearls:delivery:deliver")
        if success then
            SendNUIMessage({
                action = "pearls:delivery:deliveryResult",
...
RegisterNUICallback("pearls:delivery:continueChoice", function(data, cb)
    EndConversation()

    if data.choice == "continue" then
        local success, result = TriggerServerCallback("pearls:delivery:next")
        if success then
            RemoveDeliveryBlip()
            RemoveDeliveryNpc()
...
function Pearls_StartDelivery(jobName)
    ...
    if loc then
        ...
        if refCoords then
            ...
            local maxDist = PearlsConfig.DeliveryStartMaxDistance or 50.0
    ...
    local requiredVehicles = PearlsConfig.DeliveryVehicleModels
    if requiredVehicles and #requiredVehicles > 0 then
        local ped = PlayerPedId()
        if not IsPedInAnyVehicle(ped, false) then
            VFW.ShowNotification({ type = "JOB", title = "Pearls", subtitle = "Livraison", image = PEARLS_LOGO,
    ...
    local success, result = TriggerServerCallback("pearls:delivery:start", jobName)
    if not success then
        VFW.ShowNotification({
            type = "JOB", title = "Pearls", subtitle = "Livraison",
            image = PEARLS_LOGO,
...
function Pearls_GetDeliveryState()
    return {
...
CreateThread(function()
    ...
    local jobNames = {}
    for jobName, _ in pairs(PearlsConfig.Locations) do
        table.insert(jobNames, jobName)
    end

    registry.register(jobNames, function(menu)
        local state = Pearls_GetDeliveryState()

        if not state.active then
            menu.Button(
                "Lancer une livraison",
                "Démarrer une mission de livraison Pearls",
                nil,
                "arrow",
                false,
                function()
...
RegisterNUICallback("bossPanel:clearPearlsLogs", function(data, cb)
    local deleted = TriggerServerCallback("pearls:delivery:clearLogs")
    if not deleted then
        VFW.ShowNotification({
            type = "JOB", title = "Pearls", subtitle = "Administration",
            image = PEARLS_LOGO,
    ...
    while true do
        ...
        if deliveryActive and deliveryClientData then
            ...
            if deliveryClientData then
                ...
                local spawnDist = PearlsConfig.DeliveryNpcSpawnDistance or 50.0
