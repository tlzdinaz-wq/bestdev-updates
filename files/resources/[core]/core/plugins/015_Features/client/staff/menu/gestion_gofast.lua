...
local function Guard()
    ...
    return perms["builder_gofast"] == true
end

local function Panel()
    local res = TriggerServerCallback("gestionGoFast:hubPanel")
    if type(res) ~= "table" or res.ok ~= true then
        return { ok = false, error = "Impossible de charger le go fast." }
    end
    return res
end
...
RegisterNuiCallback("gestion:gofast:open", function(_, cb)
    if not Guard() then
        cb({ ok = false, error = "Vous n'avez pas la permission de gérer le go fast." })
        return
    end
    cb(Panel())
end)

RegisterNuiCallback("gestion:gofast:refresh", function(_, cb)
    if not Guard() then cb({ ok = false }) return end
    cb(Panel())
end)

RegisterNuiCallback("gestion:gofast:here", function(_, cb)
    if not Guard() then cb({ ok = false }) return end
    cb({ ok = true, coords = Here() })
end)

RegisterNuiCallback("gestion:gofast:teleport", function(data, cb)
    if not Guard() then cb({ ok = false }) return end
    ...
    VFW.ShowNotification({ type = "STAFF", variant = "SUCCESS", subtitle = "Go Fast", message = "Téléporté." })
    cb(Panel())
end)

RegisterNuiCallback("gestion:gofast:save", function(data, cb)
    if not Guard() then cb({ ok = false }) return end
    local result = TriggerServerCallback("gestionGoFast:save", data)
    ...
    VFW.ShowNotification({ type = "STAFF", variant = "SUCCESS", subtitle = "Go Fast", message = verb })
    result.ok = true
    cb(result)
end)
