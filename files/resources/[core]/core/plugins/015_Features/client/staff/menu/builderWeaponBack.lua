...
local function BuildWeaponBackMenu()
    if not StaffMenu or not StaffMenu.builderWeaponBack then return end

    local allowed = GlobalState.WeaponBackAllowed or {}
    local Button = StaffMenu.builderWeaponBack.Button
    local Separator = StaffMenu.builderWeaponBack.Separator
    ...
    Button("Rechercher une arme", searchText ~= "" and ("Recherche : " .. searchText) or nil, nil, "search", false, function()
        ...
        if input then
            searchText = input
            StaffMenu.builderWeaponBack.refresh()
        end
    end)
    ...
    Button("Tout activer", totalAllowed .. (totalAllowed > 1 and " armes autorisées" or " arme autorisée"), nil, "check", false, function()
        local success = TriggerServerCallback("vfw:weaponBack:setAll", true)
        if success then
            VFW.ShowNotification({ type = 'STAFF', variant = 'SUCCESS', subtitle = 'Armes dans le dos', message = "Toutes les armes activées." })
            Wait(200)
            StaffMenu.builderWeaponBack.refresh()
        end
    end)
    ...
    Button("Tout désactiver", nil, nil, "trash", false, function()
        local success = TriggerServerCallback("vfw:weaponBack:setAll", false)
        if success then
            VFW.ShowNotification({ type = 'STAFF', variant = 'SUCCESS', subtitle = 'Armes dans le dos', message = "Toutes les armes désactivées." })
            Wait(200)
            StaffMenu.builderWeaponBack.refresh()
        end
    end)
        ...
        if #weapons > 0 then
            ...
            if #filtered > 0 then
                ...
                for _, w in ipairs(filtered) do
                  ...
                  local desc = isAllowed and "Autorisée dans le dos" or "Non autorisée"
                  Button(w.label, desc, nil, icon, false, function()
                        local newState = TriggerServerCallback("vfw:weaponBack:toggle", w.name)
                        if newState ~= nil then
                            local stateText = newState and "autorisée" or "retirée"
                          VFW.ShowNotification({ type = 'STAFF', variant = 'SUCCESS', subtitle = 'Armes dans le dos', message = w.label .. " " .. stateText .. "." })
                            Wait(200)
                            StaffMenu.builderWeaponBack.refresh()
                        end
                    end)
...
if StaffMenu.builderWeaponBack and StaffMenu.builderWeaponBack.OnOpen then
    StaffMenu.builderWeaponBack.OnOpen(function()
        StaffMenu.builderWeaponBack.ClearItems()
        BuildWeaponBackMenu()
    end)
end
