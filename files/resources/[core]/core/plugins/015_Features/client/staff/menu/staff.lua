...
function StaffMenu.BuildStaffDetailsMenu()
    ...
    if selectedStaff.online and selectedStaff.source then
        StaffMenu.staffDetails.Button(":report: MENU JOUEUR COMPLET", "Ouvrir le menu joueur complet pour ce membre du staff en ligne", nil, "chevron", false, function()
            StaffMenu.data.playerInfo = TriggerServerCallback("vfw:staff:getPlayerInfo", selectedStaff.source) or {}
            StaffMenu.data.selectedPlayer = selectedStaff.source
            ...
            local playerTitle = string.format("%s [%d]", StaffMenu.data.playerInfo.name or "Unknown", selectedStaff.source)
