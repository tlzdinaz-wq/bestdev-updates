...
local Drilling = {
    ...
    drillProp = "hei_prop_heist_drill",
...
local function StartDrillingMinigame(safePos, callback)
    ...
    Citizen.CreateThread(function()
        ...
        if not dictLoaded or not drillHash or not scaleform then
            Drilling.isActive = false
            VFW.Nui.HudVisible(true)
        ...
        instructionalButtons[drillButtonId] = {
            ...
            { control = 237, label = "Pousser" },
            { control = 238, label = "Tirer" },
        ...
        while Drilling.isActive and Drilling.ScalformMoov and HasScaleformMovieLoaded(Drilling.ScalformMoov) do
            Citizen.Wait(0)
            ...
            -- Maintenir l'animation en boucle
            ...
            -- Désactiver le HUD complet (minimap + tous les composants)
            ...
            if VFW.Interact.JustPressed(0, 51) and not RaterScal then
                ...
                -- Gerer le son synchronise quand on allume/eteint la perceuse
                if SoundScal and not drillSoundPlaying then
                    -- Allumer la perceuse -> demarrer le son
                ...
                elseif not SoundScal and drillSoundPlaying then
                    -- Eteindre la perceuse -> arreter le son
                    if StopSyncedDrillSound then
                        StopSyncedDrillSound()
                    end
            ...
            if Scal3 > 0.7 and not RaterScal then
                ...
                -- Arreter le son synchronise quand la perceuse surchauffe
...
function StartPacificDrilling(safePos, callback)
    StartDrillingMinigame(safePos, callback)
end

function IsPacificDrilling()
