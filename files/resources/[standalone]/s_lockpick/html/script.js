...
$(function () {
    ...
    window.addEventListener('message', function(event){
        ...
        if (eventData.action == "ui") {
            if (eventData.toggle) {
                ...
                reset();
                $('#wrap').fadeIn(250);
                $('#hint-panel').fadeIn(250);
                gameOver = false
                gamePaused = false
            } else {
                $('#wrap').stop(true, true).fadeOut(50);
                $('#hint-panel').stop(true, true).fadeOut(50);
                gameOver = true
                gamePaused = false
            }
        }
    ...
    document.onkeyup = function (data) {
        if (data.which == 27 || data.which == 88) {
            if (gameOver) return;
            gameOver = true;
            closeUI();
            postNui('exit', {});
...
function postNui(endpoint, data) {
    fetch('https://s_lockpick/' + endpoint, {
        method: 'POST',
...
function outOfPins() {
    ...
    gameOver = true;
    closeUI();
    postNui('callback', { success: false });
    setTimeout(function(){
        reset()
    }, 250)
}
