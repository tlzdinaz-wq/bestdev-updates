...
local frameworks = { 'core', 'es_extended', 'ND_Core', 'ox_core', 'qbx_core' }
local frameworkFiles = { core = 'vfw' }
local sucess = false
...
for i = 1, #frameworks do
	...
	if GetResourceState(framework):find('start') then
		require(('server.framework.%s'):format(frameworkFiles[framework] or framework:lower()))
		sucess = true
		break
	end
end
