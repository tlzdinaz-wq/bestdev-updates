...
lib.callback.register('ox_doorlock:inputPassCode', function()
	local VFW = exports["core"]:getSharedObject()
	if not VFW or not VFW.Nui or not VFW.Nui.KeyboardInput then
		local result = lib.inputDialog(locale('door_lock'), {
			{
				type = 'input',
	...
	return VFW.Nui.KeyboardInput(true, "Code d'accès")
end)
