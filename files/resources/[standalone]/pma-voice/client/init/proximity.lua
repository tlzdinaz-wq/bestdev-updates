...
function addNearbyPlayers()
	...
	if not assignedChannel or assignedChannel == 0 then return end
	if MumbleGetVoiceChannelFromServerId(playerServerId) ~= assignedChannel then return end
...
function setSpectatorMode(enabled)
	...
	if isListenerEnabled then
		for i = 1, #players do
			...
			if not channel or channel == 0 then goto skip_loop end
...
CreateThread(function()
	TriggerEvent('chat:addSuggestion', '/mute', 'Rendre muet un joueur (voix)', {
		{ name = "id", help = "ID du joueur" },
		{ name = "duree", help = "(opt) durée du mute en secondes (défaut: 900, 0 = permanent)" }
	})
	TriggerEvent('chat:addSuggestion', '/unmute', 'Rendre la voix à un joueur', {
		{ name = "id", help = "ID du joueur" }
	})
	while true do
