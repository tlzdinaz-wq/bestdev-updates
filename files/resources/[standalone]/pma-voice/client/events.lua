...
AddEventHandler('mumbleConnected', function(address, isReconnecting)
	...
	local targetAddr = GetConvar('voice_externalAddress', '')
	local targetPort = GetConvarInt('voice_externalPort', 0)
	if targetAddr ~= '' and targetPort ~= 0 then
		local expected = targetAddr .. ':' .. tostring(targetPort)
		if tostring(address) ~= expected then
			MumbleSetServerAddress(targetAddr, targetPort)
		end
	end
