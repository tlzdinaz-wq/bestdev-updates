...
class SoundPlayer {
    ...
    setVolume(result) {
        ...
        // Appliquer le volume master global
        const finalVolume = volume * (typeof masterVolume !== 'undefined' ? masterVolume : 1.0);

        if (this.isAudioYoutubePlayer() && this.yPlayer && this.isYoutubePlayerReady()) {
            this.yPlayer.setVolume(finalVolume * 100);
        } else if (this.audioPlayer) {
            this.audioPlayer.volume(finalVolume);
        }
    }
    ...
    setTimeStamp(time) {
        if (!this.isAudioYoutubePlayer()) {
            if (this.audioPlayer != null && typeof this.audioPlayer.seek === "function") {
                this.audioPlayer.seek(time);
            }
        } else {
            if (this.isYoutubePlayerReady() && this.yPlayer && typeof this.yPlayer.seekTo === "function") {
