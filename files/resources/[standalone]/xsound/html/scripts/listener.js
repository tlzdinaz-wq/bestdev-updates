...
var masterVolume = 1.0; // Volume master global (0.0 à 1.0)
    ...
    window.addEventListener('message', function (event) {
        ...
        switch (item.status) {
            ...
            case "muteAll":
                ...
                break;
            case "masterVolume":
                masterVolume = Math.max(0, Math.min(1, item.volume));
                // Réappliquer le volume à tous les sons actifs
                for (let soundName in soundList) {
                    sound = soundList[soundName];
            ...
            case "getCurrentTime":
                ...
                if (sound != null) {
                    ...
                    if (sound.isAudioYoutubePlayer()) {
                        if (sound.isYoutubePlayerReady() && sound.yPlayer) {
                            currentTime = sound.yPlayer.getCurrentTime() || 0;
                        }
                    } else {
                        if (sound.audioPlayer != null) {
                            currentTime = sound.audioPlayer.seek() || 0;
                        }
                    }
                    $.post('https://xsound/currentTime', JSON.stringify({
                        id: item.name,
                        time: currentTime
                    }));
                }
